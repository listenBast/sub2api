...
每次发布在当前小版本上加 1：`v0.1.1`、`v0.1.2` … 直到 `v0.1.100`，
然后进入 `v0.2.0`、`v0.2.1` … 直到 `v0.2.100`，再进入 `v0.3.0`。
源码文件 `backend/cmd/server/VERSION` 不包含 `v` 前缀，例如填写 `0.1.31`。
...
团队余额双账户和 API Key 扣费模式已纳入 `v0.1.31`。本站当前不开放生图，批量生图及其他生图相关计费行为本次保持原样，后续开放生图时再单独设计和验证团队余额支持。
