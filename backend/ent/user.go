...
type User struct {
	...
	// TeamBalance holds the value of the "team_balance" field.
	TeamBalance float64 `json:"team_balance,omitempty"`
...
type UserEdges struct {
	...
	// OwnedTeam holds the value of the owned_team edge.
	OwnedTeam *Team `json:"owned_team,omitempty"`
	// TeamMembership holds the value of the team_membership edge.
...
// OwnedTeamOrErr returns the OwnedTeam value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e UserEdges) OwnedTeamOrErr() (*Team, error) {
	if e.OwnedTeam != nil {
		return e.OwnedTeam, nil
	} else if e.loadedTypes[13] {
		return nil, &NotFoundError{label: team.Label}
	}
	return nil, &NotLoadedError{edge: "owned_team"}
}

// TeamMembershipOrErr returns the TeamMembership value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e UserEdges) TeamMembershipOrErr() (*TeamMembership, error) {
	if e.TeamMembership != nil {
		return e.TeamMembership, nil
	} else if e.loadedTypes[14] {
		return nil, &NotFoundError{label: teammembership.Label}
	}
	return nil, &NotLoadedError{edge: "team_membership"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldBalance, user.FieldFrozenBalance, user.FieldTeamBalance, user.FieldBalanceNotifyThreshold, user.FieldTotalRecharged:
...
func (_m *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldTeamBalance:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field team_balance", values[i])
			} else if value.Valid {
				_m.TeamBalance = value.Float64
			}
...
// QueryOwnedTeam queries the "owned_team" edge of the User entity.
func (_m *User) QueryOwnedTeam() *TeamQuery {
	return NewUserClient(_m.config).QueryOwnedTeam(_m)
}

// QueryTeamMembership queries the "team_membership" edge of the User entity.
func (_m *User) QueryTeamMembership() *TeamMembershipQuery {
	return NewUserClient(_m.config).QueryTeamMembership(_m)
}
...
func (_m *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("team_balance=")
	builder.WriteString(fmt.Sprintf("%v", _m.TeamBalance))
	builder.WriteString(", ")
