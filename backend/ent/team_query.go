...
type TeamQuery struct {
	...
	withTransactions *TeamTransactionQuery
...
func (_q *TeamQuery) QueryTransactions() *TeamTransactionQuery {
	query := (&TeamTransactionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TeamQuery) WithTransactions(opts ...func(*TeamTransactionQuery)) *TeamQuery {
	query := (&TeamTransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withTransactions; query != nil {
		if err := _q.loadTransactions(ctx, query, nodes,
			func(n *Team) { n.Edges.Transactions = []*TeamTransaction{} },
			func(n *Team, e *TeamTransaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadTransactions(ctx context.Context, query *TeamTransactionQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamTransaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teamtransaction.FieldTeamID)
	}
	query.Where(predicate.TeamTransaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.TransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
