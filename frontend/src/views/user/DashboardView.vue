  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <template v-else-if="stats">
        <UserDashboardStats :stats="stats" :balance="dashboardBalance" :is-simple="authStore.isSimpleMode" :platform-quotas="platformQuotas" />
...
const dashboardBalance = computed(() =>
  teamStore.isOwner
    ? (teamStore.context?.team?.total_balance ?? user.value?.balance ?? 0)
...
const refreshAll = (refreshTeam = true) => {
  if (refreshTeam && teamStore.isOwner) void teamStore.fetchContext(true)
  loadStats()
  loadCharts()
  loadRecent()
  loadPlatformQuotas()
...
onMounted(async () => {
  try {
    ...
    console.error('Failed to resolve team mode:', error)
  }
  refreshAll(false)
})
</script>
